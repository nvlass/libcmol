 ...
 *
 * Supports GGUF versions 2 and 3 (v1 is rejected).
 * Little-endian only — matches all supported targets (x86, ARM).
 */
...
/* GGUF metadata value types (from spec) */
typedef enum {
    GV_UINT8   =  0,
    GV_INT8    =  1,
    GV_UINT16  =  2,
...
/* Byte widths for scalar GV types (index = gv_type_t value; 0 = variable) */
static const uint8_t GV_WIDTH[] = {1,1,2,2,4,4,4,1,0,0,8,8,8};

/* =========================================================================
 * Metadata keys we extract
 *
 * Architecture (LLaMA family, which covers SmolLM3)
 * ====================================================================== */

#define K_ARCH        "general.architecture"
#define K_CTX         "llama.context_length"
#define K_EMB         "llama.embedding_length"
#define K_LAYERS      "llama.block_count"
#define K_FFN         "llama.feed_forward_length"
#define K_HEADS       "llama.attention.head_count"
#define K_KV_HEADS    "llama.attention.head_count_kv"
#define K_ROPE_BASE   "llama.rope.freq_base"
#define K_RMS_EPS     "llama.attention.layer_norm_rms_epsilon"
#define K_VOCAB_SIZE  "llama.vocab_size"
 ...
 *
 * Sticky error: once c->err is set, all reads return 0 / NULL and the
 * caller can check at the end rather than after every single read.
 * ====================================================================== */
...
/* Scalar read via memcpy — safe on unaligned pointers */
 ...
 * Others (u8, i8, u16, i16, i64, f64) will be added in the phases
 * that need them (quant.c for the quantisation-type readers). */
...
/*
 * gcur_str — read a GGUF string (u64 length + raw bytes) and copy it into
 * the arena as a null-terminated C string.
 * Returns NULL if the arena is exhausted or the cursor is in error.
 */
...
/* gcur_str_skip — discard a GGUF string without storing it */
...
/* =========================================================================
 * Value skipping — used for unrecognised KV keys
 * ====================================================================== */
...
typedef struct {
    const char **strings;   /* arena-allocated array of merge strings     */
    int          count;
} gguf_merges_t;
...
/* =========================================================================
 * Merge resolution: "piece_a piece_b" strings → (left_id, right_id) pairs
 *
 * Strategy: build a sorted index over vocab, binary-search each piece.
 * O(V log V) for the sort + O(M log V) for lookups — fast enough.
 *
 * g_rsort_vocab is used only inside cmol_load() which is single-threaded.
 * ====================================================================== */

static const char **g_rsort_vocab; /* temporary, set before qsort call    */
...
static int32_t vocab_lookup(const int32_t *idx, int n,
                             const char *piece) {
    int lo = 0, hi = n - 1;
    while (lo <= hi) {
        int mid = (lo + hi) >> 1;
...
static cmol_err_t resolve_merges(cmol_arena_t     *a,
    ...
    /* Build sorted index (stays in arena after use — no free needed) */
    int32_t *idx = (int32_t *)cmol_arena_alloc_n(a, (size_t)tok->vocab_size,
                                                   sizeof(int32_t));
    ...
    tok->merge_left  = (int32_t *)cmol_arena_alloc_n(a, (size_t)raw->count,
                                                       sizeof(int32_t));
    tok->merge_right = (int32_t *)cmol_arena_alloc_n(a, (size_t)raw->count,
                                                       sizeof(int32_t));
    ...
    for (int i = 0; i < raw->count; i++) {
        ...
        if (!sep) {
            /* Malformed merge entry — mark as invalid */
            tok->merge_left[i] = tok->merge_right[i] = -1;
            continue;
        }
...
static cmol_err_t read_kv(gcur_t *c, cmol_arena_t *a,
    ...
    if (vtype == GV_UINT32) {
        ...
        if (c->err) return CMOL_ERR_INVALID;
        if      (!strcmp(key, K_CTX))       hp->model_max_ctx = (int)v;
        else if (!strcmp(key, K_EMB))       hp->d_model       = (int)v;
        else if (!strcmp(key, K_LAYERS))    hp->n_layers      = (int)v;
        else if (!strcmp(key, K_FFN))       hp->d_ffn         = (int)v;
        else if (!strcmp(key, K_HEADS))     hp->n_heads       = (int)v;
        else if (!strcmp(key, K_KV_HEADS))  hp->n_kv_heads    = (int)v;
        else if (!strcmp(key, K_VOCAB_SIZE))hp->vocab_size    = (int)v;
    ...
    if (vtype == GV_FLOAT32) {
        float v = gcur_f32(c);
        if (c->err) return CMOL_ERR_INVALID;
        if      (!strcmp(key, K_ROPE_BASE)) hp->rope_freq_base = v;
        else if (!strcmp(key, K_RMS_EPS))   hp->rms_norm_eps   = v;
        return CMOL_OK;
    }
...
static cmol_err_t read_tensor_info(gcur_t *c, cmol_arena_t *a,
                                    cmol_tensor_t *t) {
    /* name */
    ...
    t->data        = NULL; /* resolved after all tensor infos are read    */

    return c->err ? CMOL_ERR_INVALID : CMOL_OK;
}

/* =========================================================================
 * cmol_gguf_parse — main entry point
 * ====================================================================== */
...
cmol_err_t cmol_gguf_parse(const cmol_mmap_t *mmap,
    ...
    for (uint64_t i = 0; i < n_kv; i++) {
        cmol_err_t e = read_kv(&c, arena, hp, tok, &raw_merges);
    ...
    /* Resolve merge strings → (left_id, right_id) pairs */
    ...
    /* Tensor data starts at the next GGUF_ALIGN boundary after tensor infos */
    ...
    /* ---- Tied embeddings --------------------------------------------- */
    /* SmolLM3 135M/360M: output.weight is absent; lm_head reuses token_embd */
...
/* =========================================================================
 * cmol_gguf_peek — read only what cmol_arena_estimate() needs
 *
 * Maps the file, reads the header + KV scalars only (skips all arrays,
 * including the large tokenizer vocab), then unmaps immediately.
...
cmol_err_t cmol_gguf_peek(const char     *path,
    ...
    for (uint64_t i = 0; i < n_kv && !c.err; i++) {
        ...
        /* Extract only the scalar uint32 / float32 values we need */
        if (have_key && vtype == GV_UINT32) {
            ...
            /* Skip everything else — including the large tokenizer arrays */
            skip_value(&c, vtype);
        }
    }
